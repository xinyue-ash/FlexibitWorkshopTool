...
Blockly.Arduino['asymmetric'] = function (block) {
  ...
  var angle_lr = Blockly.Arduino.valueToCode(
    block, 'DEGREELR', Blockly.Arduino.ORDER_ATOMIC) || '90'; // angle left to right

  var angle_rl = Blockly.Arduino.valueToCode(
    block, 'DEGREERL', Blockly.Arduino.ORDER_ATOMIC) || '0'; // angle right to left
  ...
  //in loop()
  var code = 'for (int c = 0; c < ' + cycles + '; c++) {\n' +
    '  for (int pos = 0; pos <= ' + angle_lr + '; pos++) { // goes from 0 degrees to ' + angle_lr + ' degrees\n' +
    '    servo_' + dropdown_pin + '.write(pos);\n' +
    '    delay(' + delayTime + '); // Slow or fast depending on the selection\n' +
    '  }\n' +
    '  for (int pos = ' + angle_lr + '; pos >=' + angle_diff +'; pos--) { '+
    '    servo_' + dropdown_pin + '.write(pos);\n' +
    '    delay(' + delayTime + '); // Slow or fast depending on the selection\n' +
    '  }\n' +
    '}\n';
